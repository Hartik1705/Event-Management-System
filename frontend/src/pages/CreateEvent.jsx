...
const CreateEvent = () => {
    ...
    const fetchUserFromCSV = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:3000/event/fetchEmails', formdata, {
                headers: {
            ...
            const { success, emails } = response.data;
            if (success) {
                setInvitedPeople((prev) => {
                    const newEmails = emails.filter((e) => !prev.includes(e));
        ...
        <div className='w-full h-dvh flex flex-row items-center justify-evenly my-10'>
            ...
            {loading ?
                ...
                <div className='w-full h-full flex flex-col items-center justify-center '>
                    <ScaleLoader width={4} height={50} color='black' loading={loading} size={400} aria-label='Loading...' />
                    <h1 className='text-lg text-slate-800 font-poppins'>Loading...</h1>
                </div>
                    ...
                    <form onSubmit={(e) => createEvent(e)} className='w-full h-full flex flex-col items-center justify-center gap-10 border-2 border-gray-300 p-7 rounded-lg hover:shadow-blue-300 hover:shadow-lg'>
                        ...
                        <Drawer open={isOpen} className='overflow-auto' onClose={toggle} direction="right" lockBackgroundScroll={true} size={500}>
                            <div className='w-full h-auto flex flex-col items-center py-5'>
                                ...
                                <div className='w-full h-auto flex flex-col items-center justify-around m-2 p-4 gap-3'>
                                    <p className='text-slate-600 text-md font-bold   px-10'>Import participants via .csv file</p>
                                    <div className='w-full flex flex-row items-center justify-between bg-slate-100 p-10'>
                                        <label className='hover:shadow-lg hover:opacity-90 w-1/2 p-3 bg-blue-600 text-white text-center font-poppins' htmlFor="file-upload"><FileUploadIcon />{file && (file.name || `Upload File`)}</label>
                                        <input id='file-upload' type='file' className='hidden' onChange={handleFileChange} accept='.csv' />

                                        <button disabled={!file} onClick={fetchUserFromCSV} className=' w-1/3 p-2  text-lg border-2 border-slate-600 hover:bg-slate-700 hover:text-white flex flex-row items-center justify-center' type='button'>
                                            Import
                                        </button>
                                    </div>
                                </div>
